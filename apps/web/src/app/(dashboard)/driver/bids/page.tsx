    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {bids.map((bid: any) => {
            const isNegotiable = bid.status === 'PENDING' || bid.status === 'COUNTERED';
            const isCancelled  = bid.shipment?.status === 'CANCELLED';

            return (
              <div key={bid.id} className={`bg-white rounded-xl border shadow-sm overflow-hidden ${
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right ml-4">
                      ...
                      {bid.status === 'PENDING' && !isCancelled && (
                        <Button
                          variant="outline"
                          size="sm"
                ...
                {/* Negotiation chat — shown while bid is still open */}
                {isNegotiable && !isCancelled && session?.user?.id && (
